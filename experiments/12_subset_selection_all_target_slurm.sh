...
#SBATCH --job-name=pretrain_subset_all
...
for k in "${K_VALUES[@]}"; do for b in "${BUDGET_VALUES[@]}"; do
    for s in "${SEEDS[@]}"; do JOBS+=("$k $b $s random none"); done
    for s in "${SEEDS[@]}"; do for e in "${EMBS[@]}"; do JOBS+=("$k $b $s ot $e"); done; done
done; done

is_done() {
    local k=$1 b=$2 s=$3 m=$4 e=$5 v=${VAL_BUDGET[$b]}
    local dir="results/subset/logs/1_pretrain_subset${b}_K${k}_${m}/bert_singlelabel"
    local suf="_subset${b}_K${k}"
    [ "$m" = ot ] && suf+="_OT_${e}_sinkhorn_log_0.01_1000_cosine_max_per_domain" || suf+="_${m}"
    suf+="_V${v}_tgtall"
...
if [ -z "$SLURM_ARRAY_TASK_ID" ]; then
    IDX=()
    for i in "${!JOBS[@]}"; do read -r k b s m e <<< "${JOBS[$i]}"; is_done "$k" "$b" "$s" "$m" "$e" || IDX+=("$i"); done
    echo "${#IDX[@]}/${#JOBS[@]} jobs remaining"
    [ ${#IDX[@]} -eq 0 ] && exit 0
    IFS=,; sbatch --array="${IDX[*]}" "$0"; exit 0
fi

read -r K BUDGET SEED METHOD EMB <<< "${JOBS[$SLURM_ARRAY_TASK_ID]}"
ARGS=(-s $SEED --subset_size $BUDGET --val_subset_size ${VAL_BUDGET[$BUDGET]}
      --num_domains $K --domain_selection_method $METHOD --tgt_domain all --start_from_epoch 20)
[ "$METHOD" = "ot" ] && ARGS+=(--ot_distance_dir ./data/geoyfcc_text/distances/ot_distance/
