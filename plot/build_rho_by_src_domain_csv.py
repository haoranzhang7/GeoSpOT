...
def per_src_domain_rhos(combined_df, min_pairs):
    ...
    for src_domain, group in combined_df.groupby('src_domain_idx'):
        ...
        linear_reg = LinearRegression().fit(group[['dist_value']], group['acc_value'])
        r2 = r2_score(group['acc_value'], linear_reg.predict(group[['dist_value']]))
        rows.append({'src_domain_idx': src_domain, 'rho': rho, 'p_value': p_value, 'r2': r2, 'n_pairs': len(group)})
...
def main():
    ...
    for embedding_type, distance_type, distance_file, method, note, lambda_weight in iter_available_combos(
        ...
        location_embedding = embedding_type.split("+")[0] if lambda_weight is not None else ""
        rows.append({
            ...
            "rho_mean": per_domain['rho'].mean(), "rho_std": per_domain['rho'].std(),
            "abs_rho_mean": per_domain['rho'].abs().mean(), "abs_rho_std": per_domain['rho'].abs().std(),
            "r2_mean": per_domain['r2'].mean(), "r2_std": per_domain['r2'].std(),
